...
# ============================================================================
# 实时调试日志 — 写入 debug.log，即使 IDA 卡死也能从外部查看最后一条日志
# ============================================================================
_DEBUG_LOG_PATH = "/Applications/IDA Professional 9.3.app/Contents/PlugIns/debug.log"

# 启动时清空旧日志，每次插件加载都从头开始
try:
    with open(_DEBUG_LOG_PATH, "w", encoding="utf-8") as _f:
        _f.write("=== INP.py Debug Log — started {} ===\n".format(
...
def _dlog(msg):
    """实时写入调试日志到 debug.log，带时间戳，立即 flush。
    即使 IDA UI 卡死，也能从终端 tail -f debug.log 看到最后一条日志，定位卡死位置。
    """
    try:
        ts = time.strftime("%H:%M:%S", time.localtime()) + ".{:03d}".format(int((time.time() % 1) * 1000))
        line = "[{}] {}\n".format(ts, msg)
...
def _decompile_safe(func_ea, flags):
    ...
    try:
        ...
        pass
    _dlog("_decompile_safe() calling ida_hexrays.decompile(%s, flags=0x%x)..." % (hex(func_ea), flags))
    t0 = time.time()
    try:
        result = ida_hexrays.decompile(func_ea, None, flags)
        _dlog("_decompile_safe() decompile(%s) returned %s in %.3fs" % (
            hex(func_ea), "OK" if result is not None else "None", time.time() - t0))
        return result
    except TypeError:
        # IDA 版本不支持 flags 参数
        _dlog("_decompile_safe() TypeError with flags, retrying without flags for %s" % hex(func_ea))
        result = ida_hexrays.decompile(func_ea)
        _dlog("_decompile_safe() decompile(%s) [no flags] returned %s in %.3fs" % (
            hex(func_ea), "OK" if result is not None else "None", time.time() - t0))
        return result
...
class _FuncExportJob(object):
    ...
    def __init__(self, export_dir, skip_existing=True, force_reexport=False):
        ...
        _dlog("_FuncExportJob.__init__() start")
        ...
        _dlog("_FuncExportJob.__init__() done — no IDA API, no ThreadPoolExecutor")
        ...
        try:
            if not self._wait_box_active:
                ...
                try:
                    ...
                    replaced = True
                    _dlog("_update_wait_box: replace_wait_box OK")
                except Exception:
                    _dlog("_update_wait_box: replace_wait_box failed, trying show_wait_box")
                # 只有 replace 失败时才创建新的等待框
                if not replaced:
                    try:
                        ida_kernwin.show_wait_box(msg)
                        _dlog("_update_wait_box: show_wait_box OK")
                    except Exception as e:
                        _dlog("_update_wait_box: show_wait_box also failed: %s" % str(e))
                self._wait_box_active = True
            else:
                ida_kernwin.replace_wait_box(msg)
        except Exception as e:
            _dlog("_update_wait_box: unexpected error: %s" % str(e))
    ...
    def _lazy_init(self):
        ...
        """
        _dlog("_lazy_init() starting — export_dir=%s" % self.export_dir)
        ensure_dir(os.path.join(self.export_dir, "decompile"))
        ensure_dir(os.path.join(self.export_dir, "disassembly"))
        _dlog("_lazy_init() dirs ensured")

        if self.force_reexport:
            processed_addrs, prev_fallback, prev_failed, prev_skipped = set(), [], [], []
            logger.info("Force re-export mode")
            _dlog("_lazy_init() force_reexport=True, skipping progress load")
        else:
            _dlog("_lazy_init() loading progress...")
            processed_addrs, prev_fallback, prev_failed, prev_skipped = load_progress(self.export_dir)
            _dlog("_lazy_init() progress loaded: %d already processed" % len(processed_addrs))

        _dlog("_lazy_init() loading crash blacklist...")
        self.crash_blacklist = load_crash_blacklist(self.export_dir)
        _dlog("_lazy_init() blacklist loaded: %d entries" % len(self.crash_blacklist))

        _dlog("_lazy_init() listing functions via idautils.Functions()...")
        t0 = time.time()
        all_funcs = list(idautils.Functions())
        _dlog("_lazy_init() got %d total functions (took %.3fs)" % (len(all_funcs), time.time() - t0))
        ...
        _dlog("_lazy_init() %d remaining functions to process" % total_remaining)

        if total_remaining == 0:
            logger.info("All functions already exported!")
            _dlog("_lazy_init() all functions already exported, nothing to do")
            return False  # 无需处理
        ...
        test_name = ida_funcs.get_func_name(test_ea)
        _dlog("_lazy_init() testing decompile on first function: %s @ %s" % (test_name, hex(test_ea)))
        try:
            if ida_hexrays:
                _dlog("_lazy_init() calling _decompile_safe()...")
                t1 = time.time()
                test_result = _decompile_safe(test_ea, self._decompile_flags)
                dt = time.time() - t1
                if test_result is not None:
                    _dlog("_lazy_init() test decompile OK, result length: %d, took %.3fs" % (len(str(test_result)), dt))
                    test_result = None  # 释放
                else:
                    _dlog("_lazy_init() test decompile returned None (will use fallback), took %.3fs" % dt)
            else:
                _dlog("_lazy_init() ida_hexrays not available, decompile disabled")
        except Exception as e:
            _dlog("_lazy_init() test decompile FAILED: %s" % str(e))

        _dlog("_lazy_init() complete — starting export of %d functions" % total_remaining)
        ...
        if self.io_executor is None:
            ...
            _dlog("_lazy_init() creating ThreadPoolExecutor with %d workers" % io_workers)
            self.io_executor = ThreadPoolExecutor(max_workers=io_workers, thread_name_prefix="INP-IO")
            _dlog("_lazy_init() ThreadPoolExecutor created OK")
    ...
    def tick(self):
        ...
        if not self._initialized:
            _dlog("_FuncExportJob.tick() #1 — lazy init starting")
            try:
                if not self._lazy_init():
                    ...
                    _dlog("_FuncExportJob: all functions already exported, done")
                    return -1
            except Exception as e:
                logger.error("Lazy init failed: %s", e, exc_info=True)
                _dlog("_FuncExportJob.tick() lazy init FAILED: %s" % str(e))
                ida_kernwin.warning("Decompile init failed!\n\n{}".format(str(e)))
                enable_undo()
                return -1
            self._initialized = True
            _dlog("_FuncExportJob.tick() lazy init OK, entering main loop")
        ...
        if self._wait_box_active and ida_kernwin.user_cancelled():
            _dlog("_FuncExportJob.tick() CANCELLED by user at %d/%d" % (self.idx, len(self.remaining_funcs)))
        ...
        if self.idx >= len(self.remaining_funcs):
            _dlog("_FuncExportJob.tick() all functions processed (%d), finishing" % self.idx)
        ...
        _dlog("_FuncExportJob.tick() [%d/%d] processing: %s @ %s" % (
            self.idx, len(self.remaining_funcs), self._current_func_name, hex(func_ea)))
        ...
        _dlog("_FuncExportJob.tick() [%d/%d] done: %s status=%s %.3fs" % (
            self.idx, len(self.remaining_funcs), self._last_func_name,
            self._last_func_status or "?", func_elapsed))
        self._current_func_name = None
        self._current_func_ea = None
        ...
        if self.idx % 50 == 0:
            ...
            _dlog("_FuncExportJob.tick() progress saved at %d/%d" % (self.idx, len(self.remaining_funcs)))
            logger.info("Progress: %d/%d functions", self.idx, len(self.remaining_funcs))

        # 每 500 个函数清理一次内存
        if self.idx % 500 == 0:
            _dlog("_FuncExportJob.tick() memory cleanup at %d" % self.idx)
            clear_undo_buffer()
            try:
                if ida_hexrays:
    ...
    def _process_one(self, func_ea):
        ...
        if fallback_reason is None:
            ...
            _dlog("_process_one [%d/%d] decompiling: %s @ %s (size=%d bytes)" % (
                self.idx, len(self.remaining_funcs), func_name, hex(func_ea), func_size))
            ...
            try:
                ...
                if dec_obj is None:
                    ...
                    _dlog("_process_one [%d] %s: decompile returned None" % (self.idx, func_name))
                else:
                    dec_str = str(dec_obj)
                    dec_obj = None
                    if dec_str and dec_str.strip():
                        ...
                        _dlog("_process_one [%d] %s: decompile OK, %d chars" % (self.idx, func_name, len(dec_str)))
                    else:
                        fallback_reason = "empty decompilation result"
                        _dlog("_process_one [%d] %s: empty result" % (self.idx, func_name))
            except ida_hexrays.DecompilationFailure as e:
                fallback_reason = "decompilation failure: {}".format(str(e))
                _dlog("_process_one [%d] %s: DecompilationFailure: %s" % (self.idx, func_name, str(e)))
            except Exception as e:
                fallback_reason = "unexpected error: {}".format(str(e))
                _dlog("_process_one [%d] %s: unexpected error: %s" % (self.idx, func_name, str(e)))
            finally:
                dec_obj = None
            ...
            if decompile_elapsed > DECOMPILE_TIME_LIMIT:
                ...
                _dlog("_process_one [%d] %s: TIMEOUT %.1fs > %ds limit, blacklisted" % (
                    self.idx, func_name, decompile_elapsed, DECOMPILE_TIME_LIMIT))
        ...
        if output_body is None:
            _dlog("_process_one [%d] %s: falling back to disassembly, reason: %s" % (
                self.idx, func_name, fallback_reason or "unknown"))
            ...
            if output_body is None:
                ...
                _dlog("_process_one [%d] %s: FAILED (disasm also failed): %s" % (self.idx, func_name, combined))
                return
            export_type = "disassembly-fallback"
            _dlog("_process_one [%d] %s: disassembly fallback OK, %d chars" % (self.idx, func_name, len(output_body)))
    ...
    def _finish(self, cancelled):
        ...
        _dlog("_FuncExportJob._finish() called, cancelled=%s, idx=%d/%d" % (
            cancelled, self.idx, len(self.remaining_funcs)))
...
class _ExportPipeline(object):
    ...
    def start(self):
        ...
        _dlog("Pipeline.start() export_dir=%s force_reexport=%s skip_auto_analysis=%s" % (
            self.export_dir, self.force_reexport, self.skip_auto_analysis))

        ensure_dir(os.path.join(self.export_dir, "decompile"))
        ensure_dir(os.path.join(self.export_dir, "disassembly"))
        ensure_dir(os.path.join(self.export_dir, "memory"))
        _dlog("Pipeline.start() dirs ensured")

        _dlog("Pipeline.start() registering timer (interval=%dms)..." % self.TIMER_INTERVAL_MS)
        self._timer = ida_kernwin.register_timer(self.TIMER_INTERVAL_MS, self._tick)
        if self._timer is None:
            _dlog("Pipeline.start() FAILED to register timer!")
            logger.error("Failed to register export pipeline timer")
            _active_pipeline = None
            return False
        _dlog("Pipeline.start() timer registered OK, id=%s" % str(self._timer))
        return True
    ...
    def _tick(self):
        ...
        # 检查用户取消
        if self._wait_box_active and ida_kernwin.user_cancelled():
            _dlog("Pipeline._tick() CANCELLED by user at phase %d/%d" % (self._phase + 1, self._total_phases))
            self._finish(cancelled=True)
            return -1

        # 所有阶段完成
        if self._phase >= self._total_phases:
            _dlog("Pipeline._tick() all phases complete")
            self._finish(cancelled=False)
            return -1
        ...
        phase_name = self._phase_names[self._phase] if self._phase < len(self._phase_names) else "?"
        _dlog("Pipeline._tick() phase %d/%d [%s] starting..." % (
            self._phase + 1, self._total_phases, phase_name))

        try:
            done = handlers[self._phase]()
        except Exception as e:
            _dlog("Pipeline._tick() phase %d [%s] EXCEPTION: %s" % (self._phase, phase_name, str(e)))
    ...
    def _tick_analysis(self):
        ...
        if ida_auto.auto_is_ok():
            _dlog("Pipeline._tick_analysis() auto_is_ok()=True, analysis complete")
    ...
    def _tick_init(self):
        ...
        _dlog("Pipeline._tick_init() checking ida_hexrays availability...")
        if ida_hexrays is None:
            self.has_hexrays = False
            _dlog("Pipeline._tick_init() ida_hexrays is None, decompile disabled")
            logger.warning("ida_hexrays module not available, skipping decompilation")
            return True
        _dlog("Pipeline._tick_init() calling init_hexrays_plugin()...")
        t0 = time.time()
        if ida_hexrays.init_hexrays_plugin():
            self.has_hexrays = True
            self._phase_names.append("Decompile")
            self._total_phases = len(self._phase_names)
            _dlog("Pipeline._tick_init() Hex-Rays init OK (%.3fs), total phases=%d" % (
                time.time() - t0, self._total_phases))
            logger.info("Hex-Rays decompiler initialized")
        else:
            self.has_hexrays = False
            _dlog("Pipeline._tick_init() Hex-Rays init FAILED, decompile disabled")
    ...
    def _update_wait_box(self, extra=""):
        ...
        try:
            if not self._wait_box_active:
                _dlog("Pipeline._update_wait_box: show_wait_box phase=%s" % phase_name)
                ida_kernwin.show_wait_box(msg)
                self._wait_box_active = True
            else:
                ida_kernwin.replace_wait_box(msg)
        except Exception as e:
            _dlog("Pipeline._update_wait_box error: %s" % str(e))
    ...
    def _tick_strings(self):
        if not self._phase_initialized:
            _dlog("Pipeline._tick_strings() initializing string iterator...")
        ...
        while batch < self.STRINGS_BATCH:
            ...
            try:
                ...
                self._str_f.close()
                self._str_f = None
                _dlog("Pipeline._tick_strings() complete: %d strings exported" % self._str_count)
    ...
    def _tick_imports(self):
        _dlog("Pipeline._tick_imports() starting...")
        export_imports(self.export_dir)
        _dlog("Pipeline._tick_imports() complete")
        return True
    ...
    def _tick_exports(self):
        _dlog("Pipeline._tick_exports() starting...")
        export_exports(self.export_dir)
        _dlog("Pipeline._tick_exports() complete")
        return True
    ...
    def _tick_pointers(self):
        if not self._phase_initialized:
            _dlog("Pipeline._tick_pointers() initializing, collecting segments...")
            self._ptr_records = []
            self._ptr_seen = set()
            ...
            self._phase_initialized = True
            _dlog("Pipeline._tick_pointers() ptr_size=%d, %d segments to scan" % (
                self._ptr_size, len(self._ptr_all_segs)))

        if self._ptr_sub_phase == 0:
            # Data xref: 每 tick 处理 PTR_HEADS_PER_TICK 个 head（带时间预算）
            if self._ptr_seg_idx >= len(self._ptr_all_segs):
                _dlog("Pipeline._tick_pointers() dxref scan complete, dref_hits=%d, switching to raw scan" % self._ptr_dref_hits)
            ...
            if self._ptr_heads_iter is None:
                ...
                _dlog("Pipeline._tick_pointers() dxref scanning segment %d/%d: %s" % (
                    self._ptr_seg_idx, len(self._ptr_all_segs), seg_name))
            ...
            # 初始化 raw pointer 扫描
            _dlog("Pipeline._tick_pointers() initializing raw pointer scan...")
            self._ptr_raw_segs = []
            for seg_ea in idautils.Segments():
                seg = ida_segment.getseg(seg_ea)
                seg_name = ida_segment.get_segm_name(seg) if seg else ""
            ...
            _dlog("Pipeline._tick_pointers() raw scan: %d segments to scan" % len(self._ptr_raw_segs))
            return False
            ...
            if self._ptr_raw_seg_idx >= len(self._ptr_raw_segs):
                _dlog("Pipeline._tick_pointers() raw scan complete, raw_hits=%d, writing output" % self._ptr_raw_hits)
            ...
            if self._ptr_raw_heads_iter is None:
                ...
                seg = ida_segment.getseg(seg_ea)
                seg_name = ida_segment.get_segm_name(seg) if seg else "?"
                _dlog("Pipeline._tick_pointers() raw scanning segment %d/%d: %s" % (
                    self._ptr_raw_seg_idx, len(self._ptr_raw_segs), seg_name))
                seg = ida_segment.getseg(seg_ea)
            ...
            # 排序并写入
            _dlog("Pipeline._tick_pointers() sorting and writing %d pointer records..." % len(self._ptr_records))
            ...
            if self._ptr_records:
                ...
                _dlog("Pipeline._tick_pointers() wrote %d pointers to file" % len(self._ptr_records))
            else:
                logger.info("No pointers found")
                _dlog("Pipeline._tick_pointers() no pointers found")
            # 释放内存
            self._ptr_records = []
            self._ptr_seen = set()
            return True
    ...
    def _tick_memory(self):
        if not self._phase_initialized:
            _dlog("Pipeline._tick_memory() initializing, collecting segments...")
            self._mem_segs = []
            for i in range(ida_segment.get_segm_qty()):
                seg = ida_segment.getnseg(i)
                if seg:
        ...
        if self._mem_seg_idx >= len(self._mem_segs):
            _dlog("Pipeline._tick_memory() complete: %d bytes, %d files" % (self._mem_total_bytes, self._mem_file_count))
    ...
    def _tick_decompile(self):
        ...
        if not self._phase_initialized:
            ...
            _dlog("Pipeline._tick_decompile() creating _FuncExportJob (lightweight, no timer)")
            ...
            _dlog("Pipeline._tick_decompile() job created, will drive via pipeline timer")
        ...
        try:
            ...
            _dlog("Pipeline._tick_decompile() job.tick() EXCEPTION: %s" % str(e))
            self._job = None
            _active_export_job = None
        ...
        if result == -1:
            ...
            _dlog("Pipeline._tick_decompile() job.tick() returned -1, decompile phase done")
            self._job = None
            _active_export_job = None
            return True  # 阶段完成
    ...
    def _finish(self, cancelled):
        global _active_pipeline
        _dlog("Pipeline._finish() called, cancelled=%s, phase=%d/%d, has_hexrays=%s" % (
            cancelled, self._phase + 1, self._total_phases, self.has_hexrays))
...
def do_export(export_dir=None, ask_user=True, skip_auto_analysis=False, worker_count=None, force_reexport=False):
    ...
    _dlog("========== do_export() called ==========")
    _dlog("do_export() export_dir=%s ask_user=%s force_reexport=%s" % (export_dir, ask_user, force_reexport))
    ...
    _dlog("do_export() WORKER_COUNT=%d cpu_count=%s" % (WORKER_COUNT, os.cpu_count()))
    ...
    _dlog("do_export() creating _ExportPipeline...")
    pipeline = _ExportPipeline(
        export_dir=export_dir,
        force_reexport=force_reexport,
        skip_auto_analysis=skip_auto_analysis,
    )
    _dlog("do_export() starting pipeline...")
    if not pipeline.start():
        _dlog("do_export() pipeline.start() FAILED!")
        enable_undo()
        ida_kernwin.warning("Failed to start export pipeline!")
        return
    _dlog("do_export() pipeline started successfully, returning (async from now on)")
...
class ExportForAIPlugmod(ida_idaapi.plugmod_t):
    ...
    def run(self, arg):
        ...
        try:
            _dlog("Plugin.run() triggered (arg=%s)" % arg)

            # 检查是否已有导出在运行
            if _active_pipeline is not None or _active_export_job is not None:
                _dlog("Plugin.run() export already in progress, aborting")
...
class ExportForAIPlugin(ida_idaapi.plugin_t):
    ...
    wanted_name = "Export for AI--- MODIF-00"
    wanted_hotkey = "Ctrl-Shift-E"

    def init(self):
        """插件初始化 - 返回 plugmod_t 实例"""
        _dlog("ExportForAIPlugin.init() called — plugin loading...")
